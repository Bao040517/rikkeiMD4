...
//                .orElseThrow(() -> new NotFoundException("Instructor not found with id: " + instructorId));
    ...
    public PageResponse<CourseResponse> getPagedCoursesByStatus(int page, int size, String sortBy, Sort.Direction direction, CourseStatus status) {
        ...
        Page<CourseResponse> courseResponses = courseRepository.findAllByStatus(status,pageable).map(course -> CourseResponse.builder()
                .id(course.getId())
                .title(course.getTitle())
                .status(course.getStatus())
                .build());

        return PageResponse<CourseResponse>builder.build();
    }
